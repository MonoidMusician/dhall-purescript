...
let Param = { functor : < F : Functor | I : {} >, index : Text }
...
let count_params
    ...
    =   λ(args : List ADT_arg)
      → List/fold
        ...
        args
        Natural
        (λ(arg : ADT_arg) → λ(r : Natural) → if is_param arg then r + 1 else r)
        0
                        ...
                        # ( merge
                            { Hole =
                                ...
                                → merge
                                  ...
                                  p.functor
                            , Param =
                                λ(p : Param) → [ < Param = p | Const : Const > ]
                            , Const =
                                λ(c : Const) → [ < Const = c | Param : Param > ]
                            }
                            arg
                          )
                    )
...
let TextLitF
    ...
    = { cases =
          [ { args =
                [ < Const = { name = "s", type = "String" } | Param : Param > ]
            , name =
                "TextLit"
            }
          , { args =
...
let LetF
    ...
    = { cases =
          [ { name =
                "LetF"
            , args =
                [ < Const = { type = "String", name = "s" } | Param : Param >
                , < Param =
                      { functor =
                          < F = { type = "Maybe" } | I : {} >
                      , index =
...
let Test
    ...
    = { cases =
          [ { args =
                [ < Const = { name = "s", type = "String" } | Param : Param >
                , param "Nothing"
                , < Const = { name = "t", type = "String" } | Param : Param >
                , < Param =
                      { functor =
                          < F = { type = "Pair" } | I : {} >
                      , index =
